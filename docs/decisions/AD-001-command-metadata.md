...
Command data is currently split across runtime metadata, launchpad orchestration,
...
- what launchpad may derive locally
...
- move launchpad-local normalization toward a thin runtime consumer, not a metadata owner
...
- keeping launchpad and reader as parallel metadata builders: preserves the current debt

## Ownership table

| Concept | Canonical owner | Current state | Notes |
| --- | --- | --- | --- |
| command id | `src/runtime/commands.js` | owned but duplicated elsewhere | `launchpad.js` re-normalizes ids into local command objects |
| command title | `src/runtime/commands.js` | owned but duplicated elsewhere | parameter commands may derive current display title from launchpad state |
...
| command variant/title override logic | split: static variants in `runtime/commands.js`, current variant selection in `src/launchpad.js` | unclear owner | static `states` belong in runtime; current selected state belongs in launchpad |
| group id | `src/runtime/groups.js` | already correctly owned | launchpad should consume via lookup |
| group title | `src/runtime/groups.js` | owned but duplicated elsewhere | launchpad feed UI consumes and decorates it |
...
- Do not add new static command metadata inside `src/launchpad.js`.
- Do not make `src/actions.js` a presentation metadata registry.
...
- Goal:
  ...
  - reduce launchpad-local command normalization to a thin consumer
- Files affected:
...
- Behavior expected to remain unchanged:
  ...
  - launchpad rendering order
- Checks to run:
  - `node tools/build.js`
  - targeted launchpad smoke check for editor/author/service contexts

### Patch 2

- Goal:
  - make runtime command placement and audience restrictions the explicit source for visibility decisions
  - keep launchpad only as evaluator of current context/state
- Files affected:
...
- Checks to run:
  - `node tools/build.js`
  - launchpad and reader regression smoke check

## Open questions

- Should parameter command ids remain launchpad-local special cases, or should a
...
- Should launchpad visibility evaluation stay in `src/launchpad.js`, or should a
