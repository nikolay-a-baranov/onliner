...
export const embed = {
  url: {
    ...
    link(value = "") {
      const parsed = embed.url.parse(String(value).replace(/&amp;/gi, "&"));
      if (!parsed) return "";
      return embed.url.clean(parsed);
    },
  },
  template: {
    instagram(value) {
  ...
  service: {
    get(value) {
      ...
      if (host === "instagram.com") return embed.template.instagram;
      if (host === "threads.com" || host === "threads.net")
        return embed.template.threads;
      if (host === "tiktok.com") return embed.template.tiktok;
      if (host === "x.com" || host === "twitter.com") return embed.template.tweet;
      if (host === "t.me") return embed.template.telegram;
      return null;
    },
  },
  build(value) {
    const parsedUrl = embed.url.parse(value);
    if (!parsedUrl) return "";
    const cleanUrl = embed.url.clean(parsedUrl);
    const builder = embed.service.get(parsedUrl);
    if (!builder) return "";
  ...
  normalize: {
    video(value) {
      return value.replace(/\[video\][\s\S]*?\[\/video\]/gi, (full) => {
        const link = full.match(/src="([^"]*youtube\.com\/embed\/[^"]*)"/i)?.[1];
        const clean = embed.url.link(link);
    ...
    instagram(value) {
      return value.replace(/\[instagram\][\s\S]*?\[\/instagram\]/gi, (full) => {
        ...
        const clean = embed.url.link(link);
        if (!clean) return full;
        return embed.template.instagram(clean);
      });
    },
    threads(value) {
      return value.replace(/\[threads\][\s\S]*?\[\/threads\]/gi, (full) => {
        ...
        const clean = embed.url.link(link);
        if (!clean) return full;
        return embed.template.threads(clean);
      });
    },
    tweet(value) {
      return value.replace(/\[tweet\][\s\S]*?\[\/tweet\]/gi, (full) => {
        const link = full.match(/href="([^"]*(x\.com|twitter\.com)[^"]*)"/i)?.[1];
        const clean = embed.url.link(link);
        if (!clean) return full;
        return embed.template.tweet(clean);
      });
    },
    tiktok(value) {
      return value.replace(/\[tiktok\][\s\S]*?\[\/tiktok\]/gi, (full) => {
        ...
        const clean = embed.url.link(link);
        if (!clean) return full;
        return embed.template.tiktok(clean) || full;
      });
    },
    run(value) {
      return [
        embed.normalize.video,
        embed.normalize.duplicatedClosing,
        embed.normalize.instagram,
        embed.normalize.threads,
        embed.normalize.tweet,
        embed.normalize.tiktok,
      ].reduce((state, step) => step(state), value);
    },
  },
  cleanup() {
    cms.editor.runContent((value) => embed.normalize.run(value));
  },
  run(value = "") {
...
const isDirectEmbedScript = (() => {
  const source = document.currentScript?.src || "";
  return /(?:^|\/)(?:dist\/)?embed\.js(?:\?|$)/i.test(source);
})();

if (isDirectEmbedScript) {
  navigator.clipboard.readText().then(embed.run).catch(() => embed.run(""));
}
