...
(()=>{
  ...
  const encode = text => Array.from(text, char => `&#${char.codePointAt(0)};`).join("");
  const encoded = text => /&#\d+;|&#x[0-9a-f]+;|&lt;|&gt;|&quot;|&amp;#/i.test(text);
  const json = text => text
    ...
    .replace(/\s+<\/p>/gi, "</p>")
    .replace(/<p>\s*<\/p>/gi, "")
    .replace(/<\/?b\b[^>]*>/gi, tag => tag[1] === "/" ? "</strong>" : "<strong>")
    .replace(/<\/?i\b[^>]*>/gi, tag => tag[1] === "/" ? "</em>" : "<em>");
  const replace = (text, tag, edit) => text.replace(new RegExp(`\\[${tag}\\]([\\s\\S]*?)\\[\\/${tag}\\]`, "g"), (full, raw) => {
    try {
    ...
    const collect = {
      ...
      "onliner-vote": data => {
        data.variants?.forEach(item => {
          if (typeof item?.description === "string") list.push(item.description);
        });
      }
    };
    Object.entries(collect).forEach(([tag, add]) => {
      text.replace(new RegExp(`\\[${tag}\\]([\\s\\S]*?)\\[\\/${tag}\\]`, "g"), (_, raw) => {
        try {
          add(JSON.parse(raw));
        } catch {}
        return _;
  ...
  const run = mode => {
    const convert = text => mode === "decode" ? json(decode(text)) : encode(text);
    ...
    const edit = {
      ...
      "onliner-vote": data => {
        data.variants?.forEach(item => {
          if (typeof item?.description === "string") item.description = convert(item.description);
        });
      }
    };
