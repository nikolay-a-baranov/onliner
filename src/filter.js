...
(async () => {
  ...
  const showProgress = (text) => {
    ...
    if (!box) {
      ...
      box.innerHTML = `
        ...
        <div style="height:8px;margin-top:6px;border:1px solid #ccd0d4;">
          <div id="bml-progress-bar" style="height:100%;width:0%;background:currentColor;"></div>
        </div>
      `;
      box.style.cssText = `
        position:fixed;
  ...
  const getChoice = async () => {
    ...
    const sectionButtons = Object.entries(sections)
      ...
      .map(
        ([key, item]) => `
        <button type="button" data-section="${key}" class="bml-btn">
    ...
    panel.innerHTML = `
      ...
      <div class="bml-box">
        <div class="bml-current">
          ${sections[section]?.icon || "📄"} ${sections[section]?.label || section}
        </div>
        ...
        <div class="bml-row">
          ...
          <button id="bml-start" type="button" class="bml-mini">✔️</button>
    ...
    const choice = await new Promise((resolve) => {
      ...
      panel.querySelector("#bml-start").onclick = () => {
        resolve({
          section,
          month: ym(),
          user: getCurrentUser(),
