export const createEditorial = (api) => {
  const editorial = {
    ...
    agentPromptValue: [
      ...
      "",
      "Используй `index.md` как маршрутизатор и следуй правилам из `payload.md`, `editorial.md`, `source.md`, `verification.md`, `rewrite.md`, `titles.md`, `output.md`, `checklist.md` и, если источник содержит данные об изображениях, `photos.md`.",
      "",
      "Заполни `draft.v1` строго по `payload.md`, `output.md` и `checklist.md`. Не выдумывай факты; всё непроверенное вынеси в `audit`. Оцени риск близкого рерайта по `rewrite.md`.",
      "",
      "Ответ дай файлом `{{draftFilename}}`. В чат не выводи содержимое JSON. Если файл создать невозможно, выведи только валидный JSON без markdown, code fence, комментариев и текста до/после JSON.",
    ].join("\n"),
    contentSelectors: [
    ...
    bind() {
      if (editorial.bound) return false;
      editorial.bound = true;
      if (location.hostname.toLowerCase() !== "web.telegram.org") return false;
